...
Router.get('/acaralistassessment', (req, res) =>{
    try{
        Connection.query("SELECT t_acara.id AS idacara, t_acara.nama AS namaacara FROM t_acara WHERE NOT status = 'hapus' ORDER BY id ASC", async (error, results) =>{
            if(error) { 
                /** kirim error */
                res.status(500).json({
                    message: error
...
Router.post('/hasilassessment', (req, res) =>{
    try{
        ...
        if(selectacara){
            Connection.query("SELECT * FROM t_acara WHERE NOT status = 'hapus' AND id = ?", [selectacara], async (error, acaraterpilih) => {
                if(error){
                    /** send error */
                    res.status(500).json({
                        message: error
                    ...
                    Connection.query("SELECT u.id AS idmahasiswa, u.unim AS nim, u.unama AS namamahasiswa, u.ufakultas AS fakultas, u.uprodi AS prodi FROM t_user u INNER JOIN t_answer a ON a.iduser = u.id INNER JOIN t_acara c ON c.id = a.idacara WHERE u.id NOT IN (SELECT mid FROM t_kesimpulan) AND a.idacara = ? GROUP BY u.id", [selectacara], async (error, resultcekmahasiswa) => {
                        if(error) {
                            ...
                            Connection.query("SELECT t_acara.id AS idacara, t_acara.nama AS namaacara FROM t_acara WHERE NOT status = 'hapus' ORDER BY id ASC", async (error, dataacara) =>{
                                if(error) {
                                    /** send error */
                                    res.status(500).json({
                                        message: error
